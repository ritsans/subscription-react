...
- **`types.ts`** - TypeScript型定義
  - `Subscription` - サブスクリプションのデータ型（支払い日情報・トライアル期間情報含む）
...
- **`SubscriptionItem.tsx`** - 個別サブスクリプション表示
  - 価格表示（月額/年額対応）
  - トライアル期間中の特別表示（「トライアル中」表示と価格取り消し線）
  - 年額の場合は月額換算も表示
  - カテゴリ表示
  - 次回支払い日と残り日数表示（色分け）
  - トライアル期間中は残り日数表示
...
- **`Summary.tsx`** - 支出サマリー表示
  - 通貨別月額・年額合計の計算・表示（トライアル期間中サービスは除外）
  - 外貨の日本円換算表示
  - 全体合計（日本円換算）
  - トライアル期間中サービス除外の注記表示
  - 為替レートキャッシュクリア機能
...
- **無料トライアル期間管理**: トライアル期間の設定と自動判定
- **支払い日管理**: 固定日・契約日ベースの2パターン

### 3. 無料トライアル期間管理
- **トライアル期間設定**: 7日・14日・30日・90日またはカスタム日数設定
- **自動判定機能**: 現在日時とトライアル期間での自動的な状態判定
- **残り日数表示**: トライアル期間の残り日数を色分け表示
- **サマリー除外**: トライアル期間中のサービスは支出合計から自動除外
- **視覚的表示**: 「トライアル中」表示と価格の取り消し線
...
- **トライアル連携**: トライアル期間終了後の初回課金日自動計算

### 5. 支出可視化
- **通貨別集計**: 月額・年額別の合計金額表示（トライアル期間中サービス除外）
- **為替換算**: 外貨の日本円換算表示
- **全体合計**: 日本円換算での総計表示
- **詳細表示**: 折りたたみ可能な詳細パネル
- **統計情報**: 登録済みサービス数の表示
- **トライアル期間表示**: トライアル中サービスの視覚的識別と注記
