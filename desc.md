...
- **`App.tsx`** - メインアプリケーションコンポーネント
  - React Router DOM v7によるルーティング設定
  - AuthProviderとQueryClientProviderの設定
  - ページベースの構造管理
...
#### pages/ ディレクトリ
- **`TopPage.tsx`** - ランディングページ（トップページ）
  - アプリケーションの紹介とナビゲーション
  - 未認証ユーザー向けの案内
- **`LoginPage.tsx`** - ログインページ
  - 認証フォームとレイアウト
  - ログイン専用ページ構成
- **`SignupPage.tsx`** - ユーザー登録ページ
  - 新規ユーザー登録フォーム
  - サインアップ専用ページ構成
- **`DashboardPage.tsx`** - メインダッシュボードページ
  - 認証済みユーザーのメイン画面
  - サブスクリプション管理の全機能
  - モーダル制御とCRUD操作
...
#### reducers/ ディレクトリ
- **`appReducer.ts`** - useReducerパターン実装（利用可能だが現在未使用）
  - アプリケーション全体のstate管理用reducer
  - サブスクリプション、モーダル、フィルタ状態の統合管理
  - 複雑な状態管理が必要な場合の準備済み実装
...
- **`LoginForm.tsx`** - ログインフォーム
  ...
  - バリデーション機能付き
- **`SignupForm.tsx`** - ユーザー登録フォーム
  - 新規ユーザー登録機能
  - Email/パスワードバリデーション
- **`ProtectedRoute.tsx`** - 認証保護されたルートコンポーネント
  - 認証状態の検証
  - 未認証時のリダイレクト処理
...
- **保護されたルート**: ProtectedRouteコンポーネントによる認証制御
- **ページベース認証**: 未認証時は各専用ページ（Top/Login/Signup）表示
